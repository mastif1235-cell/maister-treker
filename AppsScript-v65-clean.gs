 ...
 * HMAC secret is stored in Script Properties as MT_HMAC_SECRET, never source.
 */
...
var SECURE_AUTH_NONCE_PREFIX='MT_HMAC_NONCE_V2_';
...
/* ---------- HMAC authentication ---------- */
function authSecret_(){return String(PropertiesService.getScriptProperties().getProperty('MT_HMAC_SECRET')||'');}
function authB64_(bytes){return Utilities.base64EncodeWebSafe(bytes).replace(/=+$/g,'');}
/* Length is checked separately, then every character is compared. */
function authEq_(a,b){a=String(a||'');b=String(b||'');var d=a.length^b.length,n=Math.max(a.length,b.length);for(var i=0;i<n;i++)d|=(a.charCodeAt(i)||0)^(b.charCodeAt(i)||0);return d===0;}
function authReady_(){return authSecret_().length>=SECURE_AUTH_MIN_SECRET_LENGTH;}
function authSig_(s){return authB64_(Utilities.computeHmacSha256Signature(String(s),authSecret_(),Utilities.Charset.UTF_8));}
...
function authConsumeNonce_(nonce){
  ...
  /* Cache entries are atomic enough for replay suppression; the script lock
   * around nonce consumption prevents two concurrent requests accepting the
   * same nonce before either writes it. */
  var lock=LockService.getScriptLock();
  try{lock.waitLock(3000);var cache=CacheService.getScriptCache(),key=SECURE_AUTH_NONCE_PREFIX+authNonceHash_(nonce);if(cache.get(key)!==null)return false;cache.put(key,'1',Math.ceil(SECURE_AUTH_NONCE_TTL_MS/1000));return true;}
...
/* ---------- Validation ---------- */
function validId_(v){var s=safeString(v);return s.length>0&&s.length<=200;}
function validTicket_(t){return t&&validId_(t.id)&&safeString(t.date).length<=40&&safeString(t.time).length<=20&&String(t.content||'').length<=200000&&String(t.fullDataJson||'').length<=2000000&&Array.isArray(t.tags||[]);}
function validShift_(s){return s&&validId_(s.id)&&safeString(s.date).length<=40&&safeString(s.coworker).length<=500&&isFinite(Number(s.hours));}
function validList_(a,max){return Array.isArray(a)&&a.length<=max;}
...
function mutateTicket_(action,t){if(!validTicket_(t))throw new Error('invalid ticket');var sheet=ensureSheet_('Заявки',TICKET_HEADERS),row=findRowById_(sheet,t.id);if(action==='deleteTicket'){if(row>0)sheet.deleteRow(row);return;}if(action==='addTicket'&&row>0)return;if(row<0)row=sheet.getLastRow()+1;writeTicket_(sheet,row,t);sortTickets_(sheet);}
function writeAllTickets_(tickets){if(!validList_(tickets,20000)||tickets.some(function(t){return !validTicket_(t);}))throw new Error('invalid tickets');var sheet=ensureSheet_('Заявки',TICKET_HEADERS),last=sheet.getLastRow();if(last>1)sheet.getRange(2,1,last-1,8).clearContent();var list=tickets.slice().sort(function(a,b){return ticketDateKey_(b)-ticketDateKey_(a);});if(!list.length)return;var rows=list.map(function(t){return[t.id,t.date,t.time,t.content,t.sum,(t.tags||[]).join(', '),t.backupNote||'',t.fullDataJson||''];});sheet.getRange(2,1,rows.length,8).setValues(rows);sheet.getRange(2,1,rows.length,3).setNumberFormat('@');sheet.getRange(2,5,rows.length,1).setNumberFormat('0.##');sheet.getRange(2,6,rows.length,3).setNumberFormat('@');sheet.getRange(2,4,rows.length,1).setWrap(true);}
...
function addShift_(s){if(!validShift_(s))throw new Error('invalid shift');var sheet=ensureSheet_('Зміни',SHIFT_HEADERS),row=findRowById_(sheet,s.id);if(row>0){writeShift_(sheet,row,s);return;}var d=parseDdMmYyyy(s.date),last=sheet.getLastRow(),insert=last+1;if(d&&last>1){var dates=sheet.getRange(2,2,last-1,1).getValues();for(var i=0;i<dates.length;i++){var existing=parseDdMmYyyy(dates[i][0]);if(existing&&existing>d){insert=i+2;break;}}}if(insert<=last)sheet.insertRowBefore(insert);writeShift_(sheet,insert,s);}
function writeAllShifts_(shifts){if(!validList_(shifts,20000)||shifts.some(function(s){return !validShift_(s);}))throw new Error('invalid shifts');var sheet=ensureSheet_('Зміни',SHIFT_HEADERS),last=sheet.getLastRow();if(last>1)sheet.getRange(2,1,last-1,4).clearContent();if(!shifts.length)return;var rows=shifts.map(function(s){return[s.id,s.date,s.hours,s.coworker];});sheet.getRange(2,1,rows.length,4).setValues(rows);sheet.getRange(2,1,rows.length,2).setNumberFormat('@');sheet.getRange(2,3,rows.length,1).setNumberFormat('0.##');sheet.getRange(2,4,rows.length,1).setNumberFormat('@');}
function listState_(){var ss=SpreadsheetApp.getActiveSpreadsheet(),ts=ss.getSheetByName('Заявки'),ssheet=ss.getSheetByName('Зміни'),tickets=[],shifts=[];if(ts&&ts.getLastRow()>1)for(var r=2;r<=ts.getLastRow();r++){var t=ticketObjectAt_(ts,r);if(t.id||t.date)tickets.push(t);}if(ssheet&&ssheet.getLastRow()>1)for(var s=2;s<=ssheet.getLastRow();s++){var sh=shiftObjectAt_(ssheet,s);if(sh.id||sh.date)shifts.push(sh);}return{status:'ok',tickets:tickets,shifts:shifts};}

function doGet(e){try{var p=e&&e.parameter||{};if(!authGet_(p))return jsonResponse({status:'error',message:'forbidden'});var action=String(p.action||'list'),tickets=sheet_('Заявки');if(action==='checkTicketExists')return jsonResponse({status:'ok',exists:findRowById_(tickets,p.id)>0});if(action==='getTicketById'){var row=findRowById_(tickets,p.id);return jsonResponse({status:'ok',ticket:row>0?ticketObjectAt_(tickets,row):null});}return jsonResponse(listState_());}catch(_){return jsonResponse({status:'error',message:'forbidden'});}}
function doPost(e){var lock=null;try{var raw=e&&e.postData?String(e.postData.contents||''):'';if(raw.length>SECURE_AUTH_MAX_BODY_CHARS*2)return jsonResponse({status:'error',message:'request too large'});var body=authPost_(JSON.parse(raw||'{}'));if(body===null)return jsonResponse({status:'error',message:'forbidden'});var data=JSON.parse(body),action=String(data.action||''),allowed={addTicket:1,updateTicket:1,deleteTicket:1,addShift:1,deleteShift:1,syncAll:1,syncAllTickets:1,syncAllShifts:1,clearAll:1};if(!allowed[action])return jsonResponse({status:'error',message:'unsupported action'});lock=LockService.getScriptLock();lock.waitLock(30000);if(action==='addTicket'||action==='updateTicket'||action==='deleteTicket')mutateTicket_(action,data);else if(action==='addShift')addShift_(data);else if(action==='deleteShift'){if(!validId_(data.id))throw new Error('invalid id');deleteById_(ensureSheet_('Зміни',SHIFT_HEADERS),data.id);}else if(action==='syncAll'){writeAllTickets_(data.tickets||[]);writeAllShifts_(data.shifts||[]);}else if(action==='syncAllTickets')writeAllTickets_(data.tickets||[]);else if(action==='syncAllShifts')writeAllShifts_(data.shifts||[]);else if(action==='clearAll'){writeAllTickets_([]);writeAllShifts_([]);}SpreadsheetApp.flush();return jsonResponse({status:'ok'});}catch(err){return jsonResponse({status:'error',message:'request failed'});}finally{if(lock)try{lock.releaseLock();}catch(_){}}}
