...
- A mutation whose `revision` is below the stored one is answered with HTTP 200 `{status:'ok', outcome:'STALE', state}` and writes nothing. Clients must not treat `outcome:'STALE'` as acknowledgement of their operation: the canonical client parks it as a conflict (see SYNC-STATE-MACHINE.md) so the unsent local change is never silently dropped.
