/* =====================================================================
   ...
   - якщо хмарний запис свіжіший за локальний — сервер повертає STALE,
     хмарна версія лишається недоторканою (звіт покаже «хмара новіша»);
...
function mtSyncFullPushReportHtml(report){
  ...
  if(queued && !report.wasOnline){
    ...
    html += `<p style="margin:0;">✅ Відправку завершено. Для ${cloudNewer} запис(ів) у хмарі виявилась свіжіша версія — вона НЕ перезаписана, локальна копія для них не застосовувалась.</p>`;
  }else{
