...
/* NEW (💰 Ціни): місто у формі змінилось — переобчислюємо автопідставлені
   суми під ефективні ціни цього населеного пункту.

   Свого механізму тут немає навмисне: обидві функції нижче — ті самі, що
   вже працюють при зміні типу заявки, і вони мовчки виходять, якщо майстер
   уже ввів суму вручну (feeIsAutoDefault / tariffIsAutoDefault = false).
   Тому ручна ціна не затирається, а старі збережені заявки не чіпаються
   взагалі — ці функції пишуть лише в живу форму. */
function applyCityPricing(){
  applyDefaultCallFee();
  applyDefaultTariff();
}
