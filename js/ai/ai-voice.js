...
/* Покрокова інструкція для Android Chrome (permission = denied) та порада
   для in-app webview. Тримається в модулі голосу як єдине джерело правди,
   UI лише рендерить. Жодних спроб програмно обійти denied — тільки
   пояснення, ДЕ саме користувач вмикає мікрофон. */
MTAI.voiceHelp = {
  denied: {
    title: 'Як дозволити мікрофон у Chrome (Android)',
    steps: [
      'Натисніть значок ліворуч від адреси сайту (🔒 або ⓘ / меню сайту).',
      'Відкрийте «Дозволи» / «Налаштування сайту».',
      'Знайдіть пункт «Мікрофон».',
      'Виберіть «Дозволити».',
      'Поверніться у Master-Tracker.',
      'Натисніть 🎤 ще раз.'
    ],
    footer: 'Якщо пункту «Мікрофон» немає — Chrome ⋮ → Налаштування → Налаштування сайтів → Мікрофон → знайдіть цей сайт.'
  },
  webview: {
    title: 'Відкрийте сторінку у Chrome',
    steps: [
      'Натисніть ⋮ у кутку вбудованого браузера.',
      'Виберіть «Відкрити у Chrome» / «Відкрити у браузері».',
      'У Chrome натисніть 🎤 ще раз.'
    ],
    footer: 'Вбудований браузер (Telegram/Viber/Facebook) може не давати доступ до мікрофона.'
  }
};
...
MTAI.createVoiceInput = function(deps){
  ...
  const MESSAGES = {
    ...
    denied: 'Мікрофон заборонено браузером. Відкрийте інструкцію нижче — як дозволити доступ у Chrome.',
    permission: 'Мікрофон заборонено браузером. Відкрийте інструкцію нижче — як дозволити доступ у Chrome.',
