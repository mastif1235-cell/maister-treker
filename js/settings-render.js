...
function renderMapMarkerPreferences(){
  ...
  const renderer=globalThis.MTMapMarkerRenderer,labels=renderer?.CATEGORIES||{private:{icon:'🏠',label:'Приватний будинок'},apartment:{icon:'🏢',label:'Багатоквартирний будинок'},FOB:{icon:'📦',label:'FOB'},'Муфта':{icon:'🔗',label:'Муфта'},'Вузол':{icon:'📡',label:'Вузол'},'Інше':{icon:'📍',label:'Інше'}};
  ...
  root.innerHTML=MAP_MARKER_CATEGORIES.map(category=>{const current=prefs[category],meta=labels[category];return `<section class="map-marker-category"><strong>${meta.icon} ${escapeHtml(meta.label)}</strong><div class="map-marker-shapes" aria-label="Вигляд мітки ${escapeHtml(meta.label)}">${shapes.map(([shape,label])=>{const choice={shape,size:current.size},src=renderer?.dataUrl?.(category,choice,settings.mapMarkerPreset)||'',descriptor=renderer?.descriptor?.(category,choice,settings.mapMarkerPreset);return `<button type="button" class="map-marker-shape ${current.shape===shape?'active':''}" data-marker-category="${escapeHtml(category)}" data-marker-shape="${shape}" aria-pressed="${current.shape===shape}" title="${label}"><img class="map-marker-preview" style="height:${Math.round((descriptor?.height||48)*.72)}px" src="${src}" alt=""><small>${label}</small></button>`;}).join('')}</div><div class="map-marker-sizes" aria-label="Розмір мітки ${escapeHtml(meta.label)}">${sizes.map(([size,label])=>`<button type="button" class="btn btn-sm ${current.size===size?'btn-accent':''}" data-marker-category="${escapeHtml(category)}" data-marker-size="${size}" aria-pressed="${current.size===size}">${label}</button>`).join('')}</div></section>`;}).join('');
}
...
const SETTINGS_HUB_SECTIONS = [
  ...
  {key:'ai', icon:'🤖', title:'AI-асистент', sub:'Питання по заявках, режим READ-ONLY', match:['AI-асистент']},
];
