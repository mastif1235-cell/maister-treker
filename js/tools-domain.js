...
function toolsOfflineAreasHtml(areas=[]){
  ...
  return `<div class="card"><strong>${installed?'✅ Офлайн-карта встановлена':'Офлайн-карта не встановлена'}</strong><div style="margin-top:9px;">${areas.map((area,index)=>{const linked=installed?.areaId===area.id;return `<div class="tools-offline-area-row"><div><strong>${index+1}. ${escapeHtml(area.name)}</strong><div class="tools-offline-map-meta">${linked?`✅ Офлайн-карта встановлена · ${escapeHtml(MTOfflineMap.formatBytes(installed.size))}`:'Збережена лише область — карти ще немає'}</div></div><div class="row wrap tools-offline-primary-actions"><button type="button" class="btn btn-sm" data-tools-action="show-offline-area" data-area-id="${escapeHtml(area.id)}">Показати межі</button><button type="button" class="btn btn-sm" data-tools-action="import-offline-area" data-area-id="${escapeHtml(area.id)}">Додати офлайн-карту (.pmtiles)</button><button type="button" class="btn btn-sm" data-tools-action="offline-map-guide" data-area-id="${escapeHtml(area.id)}">Як отримати .pmtiles?</button></div><details class="tools-offline-more"><summary>Ще</summary><div class="row wrap"><button type="button" class="btn btn-sm" data-tools-action="edit-offline-area" data-area-id="${escapeHtml(area.id)}">Змінити межі</button><button type="button" class="btn btn-sm" data-tools-action="export-offline-area" data-area-id="${escapeHtml(area.id)}">Зберегти межі області (.json)</button><button type="button" class="btn btn-sm btn-danger" data-tools-action="delete-offline-area" data-area-id="${escapeHtml(area.id)}">Видалити</button></div><div class="tools-offline-map-meta">Це лише межі та масштаб, не файл карти. JSON не містить плиток.</div></details></div>`;}).join('')}</div></div>`;
}
...
function toolsSaveOfflineArea(){
  ...
  if(toolsSaveOfflineAreas(areas)){toolsOfflinePendingBounds=null;toolsOfflineEditingAreaId='';renderToolsScreen('offline');showToast('✅ Межі області збережено');}
}
