...
function mtBackupIosStandalone(){
  ...
  async function mtBackupPasswordInteractive(confirmNew){
    // Модальне вікно застосунку працює і в standalone iOS, і у звичайному браузері.
    // Native prompt лишається лише аварійним fallback до появи UI.
    if(typeof openModal==='function')return await mtBackupPasswordModal(confirmNew);
  ...
  renderBackupPasswordStatus=async function(){const saved=!!(await mtBackupVaultRead());const status=document.getElementById('backupPasswordStatus');const save=document.getElementById('backupPasswordSaveBtn');const change=document.getElementById('backupPasswordChangeBtn');const forget=document.getElementById('backupPasswordForgetBtn');if(status)status.textContent=saved?'✅ Пароль для резервних копій збережено на цьому пристрої':'Пароль для резервних копій ще не збережено';if(save)save.classList.toggle('hidden',saved);if(change)change.classList.toggle('hidden',!saved);if(forget)forget.classList.toggle('hidden',!saved);return saved;};
  ...
  // Baseline синхронізації (серверна revision/tombstone кожного запису та
  // невідправлені мутації). Без нього база, відновлена на чистому пристрої,
  // не знає серверних ревізій: перша ж правка старої заявки йде як addTicket
  // revision 1, сервер (уже на revision N) відповідає STALE і правка не
  // потрапляє в Google Sheets. Той самий знімок, що й у pre-restore копії
  // (restore-from-sheets.js); mtBackupRestore застосовує його, якщо він є.
  // Без рушія (деградований режим) поле не пишемо: порожній журнал у бекапі
  // при відновленні стер би чужий baseline.
  function mtBackupJournalSnapshot(){try{const engine=typeof syncEngine!=='undefined'?syncEngine:null;if(!engine||!engine.state||typeof engine.state!=='object'||!engine.state.records)return null;return JSON.parse(JSON.stringify(engine.state));}catch(_e){return null;}}
  function mtBackupWithJournal(payload){const journal=mtBackupJournalSnapshot();if(journal)payload.syncJournal=journal;return payload;}
  function mtExternalDailyPayload(){const tools=typeof toolsExportData==='function'?toolsExportData():{};return mtBackupWithJournal({app:'master-tracker',backupVersion:6,exportedAt:new Date().toISOString(),tickets:mtBackupSafeExport(mtBackupStripPhotoData(tickets||[])),shifts:mtBackupSafeExport(mtBackupStripPhotoData(shifts||[])),settings:securitySanitizeSettingsForBackup(settings),diagnostics:mtBackupSafeExport(tools.diagnostics||[]),networkPoints:mtBackupSafeExport(tools.networkPoints||[]),secretsExcluded:true});}
