...
if(typeof window!=='undefined'){
  ...
  function mtExternalDailyPayload(){const tools=typeof toolsExportData==='function'?toolsExportData():{};return{app:'master-tracker',backupVersion:6,exportedAt:new Date().toISOString(),tickets:mtBackupStripPhotoData(tickets||[]),shifts:mtBackupStripPhotoData(shifts||[]),settings:securitySanitizeSettingsForBackup(settings),diagnostics:tools.diagnostics||[],networkPoints:tools.networkPoints||[],secretsExcluded:true};}
  ...
  exportJsonBackup=async function(){const password=await mtBackupPasswordForExport();if(!password)return;try{const tools=typeof toolsExportData==='function'?toolsExportData():{},photos=await collectLocalPhotoData(tickets.concat(typeof toolsPhotoOwnerRecords==='function'?toolsPhotoOwnerRecords():[]));const payload={app:'master-tracker',backupVersion:6,exportedAt:new Date().toISOString(),tickets,shifts,settings:securitySanitizeSettingsForBackup(settings),diagnostics:tools.diagnostics||[],networkPoints:tools.networkPoints||[],photoData:photos.photoData};const envelope=await MTBackupSystem.encrypt(payload,password);mtBackupDownload(envelope,`master-tracker-backup-${localDateKey(new Date())}-encrypted.json`);showToast(photos.missingPhotos?`Бекап зашифровано; не знайдено фото: ${photos.missingPhotos}`:'🔐 Бекап зашифровано AES-GCM');}catch(_e){showToast('Не вдалося створити зашифрований бекап');}};
  handleJsonImportFile=async function(file){if(!file)return;if(file.size>MTBackupSystem.MAX_FILE_BYTES){showToast('Файл бекапу завеликий');return;}try{const parsed=JSON.parse(await file.text());let payload;if(parsed?.format===MTBackupSystem.FORMAT){payload=await mtBackupDecryptForImport(parsed);if(!payload)return;}else{if(!MTBackupSystem.validatePayload(parsed))throw new Error('BAD_LEGACY');if(!confirm('Це legacy незашифрований бекап. Імпортувати після перевірки схеми?'))return;payload=parsed;}if(await mtBackupRestore(payload))showToast('✅ Бекап відновлено');}catch(_e){showToast('❌ Невірний пароль, пошкоджений або небезпечний файл');}};
