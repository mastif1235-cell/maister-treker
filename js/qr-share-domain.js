...
function printDogovorAsPdf({address,login,password,contractNumber}){
  const scheduleHtml=escapeHtml(LNET_CONTACTS.schedule).replace(/\n/g,'<br>'),printHtml=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Договір LNET</title><style>body{font-family:Arial,Helvetica,sans-serif;color:#111;padding:28px}h1{font-size:19px;margin:0 0 4px}.date{font-size:12px;color:#555;margin-bottom:18px}.row{margin-bottom:12px}.label{font-weight:700;font-size:11.5px;color:#555;text-transform:uppercase}.value{font-size:15px;margin-top:2px}hr{border:none;border-top:1px solid #ccc;margin:16px 0}.footer{margin-top:26px;padding-top:14px;border-top:2px solid #111;font-size:12.5px;color:#333}.footer a{color:#0a5;word-break:break-all}.footer .big{font-weight:700;font-size:14px}</style></head><body><h1>LNET — Договір на підключення</h1><div class="date">${escapeHtml(formatUaDate(new Date()))}${contractNumber?' · № '+escapeHtml(contractNumber):''}</div><div class="row"><div class="label">Адреса підключення</div><div class="value">${escapeHtml(address)}</div></div><div class="row"><div class="label">Логін</div><div class="value">${escapeHtml(login)}</div></div><div class="row"><div class="label">Пароль</div><div class="value">${escapeHtml(password)}</div></div><div class="row"><div class="label">Особистий рахунок</div><div class="value">${escapeHtml(login)}</div></div><hr><div class="row"><div class="label">Контакти</div><div class="value">${escapeHtml(LNET_CONTACTS.phone)}, ${escapeHtml(LNET_CONTACTS.viber)}</div></div><div class="row"><div class="label">Графік роботи</div><div class="value">${scheduleHtml}</div></div><div class="footer"><div class="big">Повний текст договору (публічна оферта)</div>Офіційна юридична копія договору та всі документи — на сайті LNET:<br><a href="https://lnet.com.ua/dokumenti/">https://lnet.com.ua/dokumenti/</a></div></body></html>`;
  const iframe=document.createElement('iframe');iframe.style.cssText='position:fixed; right:0; bottom:0; width:0; height:0; border:0;';document.body.appendChild(iframe);const doc=iframe.contentWindow.document;doc.open();doc.write(printHtml);doc.close();iframe.onload=()=>{try{iframe.contentWindow.focus();iframe.contentWindow.print();}catch(_e){showToast('Не вдалося відкрити друк — спробуйте ще раз');}setTimeout(()=>iframe.remove(),1500);};
}
