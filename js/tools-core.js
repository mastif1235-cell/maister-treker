(function(root,factory){
  ...
  async function runBrowserSpeedTest(options={}){
    ...
    options.onProgress?.('download');const downSamples=[];let downBytes=0,downElapsed=0,probe=canContinue()?await download(Math.max(250000,Number(options.downloadBytes)||1000000)):{ok:false,cancelled:true};if(probe.ok){downSamples.push(probe.mbps);downBytes+=probe.bytes;downElapsed+=probe.elapsed;}const downChunk=probe.mbps>=300?40000000:probe.mbps>=100?15000000:probe.mbps>=20?5000000:1000000,downBudget=Math.max(downChunk,Number(options.maxDownloadBytes)||160000000);for(let index=0;index<5&&canContinue()&&downBytes+downChunk<=downBudget&&(downElapsed<2500||downSamples.length<3);index++){const sample=await download(downChunk);if(!sample.ok)break;downSamples.push(sample.mbps);downBytes+=sample.bytes;downElapsed+=sample.elapsed;options.onSample?.('download',sample);}
    options.onProgress?.('upload');const upSamples=[];let upBytes=0,upElapsed=0,upProbe=canContinue()?await upload(Math.max(100000,Number(options.uploadBytes)||500000)):{ok:false,cancelled:true};if(upProbe.ok){upSamples.push(upProbe.mbps);upBytes+=upProbe.bytes;upElapsed+=upProbe.elapsed;}const upChunk=upProbe.mbps>=200?25000000:upProbe.mbps>=50?10000000:upProbe.mbps>=10?2500000:500000,upBudget=Math.max(upChunk,Number(options.maxUploadBytes)||75000000);for(let index=0;index<4&&canContinue()&&upBytes+upChunk<=upBudget&&(upElapsed<2500||upSamples.length<3);index++){const sample=await upload(upChunk);if(!sample.ok)break;upSamples.push(sample.mbps);upBytes+=sample.bytes;upElapsed+=sample.elapsed;options.onSample?.('upload',sample);}
    ...
    return sanitizeDiagnosticResult({online:available>0,internetStatus:available>0?'ok':'limited',dnsStatus:available>0?'indirect':'limited',summaryStatus:downloadMbps!==null&&uploadMbps!==null?'ok':available?'warning':'offline',downloadMbps,uploadMbps,latencyMs,jitterMs,speedProvider:'Cloudflare',speedMethod:'Cloudflare adaptive browser test v2',speedStatus:cancelled?'cancelled':downloadMbps!==null&&uploadMbps!==null?'success':available?'partial':'error',resources:[{label:'Cloudflare Speed',ok:downloadMbps!==null||uploadMbps!==null,state:cancelled?'unavailable':downloadMbps!==null||uploadMbps!==null?'ok':'blocked',detail:cancelled?'Скасовано користувачем':downloadMbps!==null&&uploadMbps!==null?`Adaptive HTTPS: отримано ${downBytes} B, відправлено ${upBytes} B`:'Частина вимірювань недоступна'}]});
  }
