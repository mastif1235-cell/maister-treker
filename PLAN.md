...
7. **Tickets — in progress.** `ticket-editor-core.js` now constructs calculator state before `app.js`; `ticket-editor-domain.js`, `tickets-domain.js` and `tickets-bindings.js` own form/draft/save, list/trash/calendar mutations and ticket/calculator event wiring. Source owners were removed immediately, load order is explicit, and the journal-before-storage calls are unchanged. Address/naryad is the remaining ticket sub-block.
