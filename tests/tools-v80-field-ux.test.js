...
assert.match(domain,/Вибрати область/);assert.match(map,/root\.L\.rectangle/);assert.match(domain,/не завантажує плитки масово з OpenStreetMap або MapTiler/);
...
assert.match(domain,/Зберегти межі області/);assert.match(domain,/toolsSaveOfflineArea/);assert.match(domain,/toolsEditOfflineArea/);assert.match(domain,/toolsDeleteOfflineArea/);assert.match(domain,/toolsImportOfflineArea/);assert.match(domain,/Показати межі/);assert.match(domain,/Збережена лише область/);assert.match(domain,/Офлайн-карта встановлена/);assert.match(map,/drawBounds/);
