...
(async()=>{
  ...
  assert.doesNotMatch(html,/vendor\/maplibre\/maplibre-gl\.css/,'MapLibre CSS is lazy, not render-blocking at boot');
  assert.doesNotMatch(html,/type="module" src="js\/tools-map-maplibre\.js"/,'MapLibre module is lazy, not parsed at boot');
  assert.match(tools,/loadMapRuntimeAsset\('maplibre-css','vendor\/maplibre\/maplibre-gl\.css','style'\)/);
  assert.match(tools,/import\(mapAssetUrl\('js\/tools-map-maplibre\.js'\)\)/);
  ...
  assert.doesNotMatch(html,/vendor\/leaflet\/leaflet\.js/,'Leaflet is fetched only after the WebGL fallback is needed');
  assert.match(tools,/loadMapRuntimeAsset\('leaflet-js','vendor\/leaflet\/leaflet\.js','script'\)/);
