...
(async()=>{
  ...
  assert.equal(calls.sources['mt-objects'].data.features[0].properties.icon,'mt-object-private-drop');
  assert.equal(calls.sources['mt-objects'].data.features[1].properties.icon,'mt-object-FOB-drop');
  assert.equal(calls.layers.find(layer=>layer.id==='mt-objects').type,'symbol');
  assert.equal(Object.keys(calls.images).length,24,'each object category has four local shape images');
