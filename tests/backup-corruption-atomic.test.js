...
(async()=>{
  ...
  assert.match(source,/openConfirmModal\(\{title:'Імпортувати незашифрований бекап\?'/,'legacy import remains explicit');assert.doesNotMatch(source,/console\.(?:log|error)\([^)]*(?:password|token|ciphertext)/i,'secret material is not logged');
