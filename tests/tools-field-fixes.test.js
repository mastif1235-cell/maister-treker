...
assert.match(domain,/Завантаження/);assert.match(domain,/Відвантаження/);assert.match(domain,/Не вдалося виміряти швидкість/);assert.match(coreSource,/speed\.cloudflare\.com/);assert.doesNotMatch(domain,/Мбіт\/с[^`]*0/,'12 speed UX has no fake empty metrics');
...
assert.match(domain,/Зберегти параметри \(\.json\)/);assert.match(domain,/JSON містить лише назву, координати меж і zoom/,'area JSON is explicitly described only inside technical help');
