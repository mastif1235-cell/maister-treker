...
assert.match(source,/CACHE_NAME\s*=\s*'maister-treker-v66-runtime-50'/,'installed-PWA cache revision is unique for v91.13');
assert.match(appSource,/APP_VERSION\s*=\s*'v91\.13 · 2026-09-08'/,'canonical release identity is v91.13');
...
assert.match(fs.readFileSync(path.join(root,'js','security-audit-fixes-v65-18-9.js'),'utf8'),/SECURITY_AUDIT_RELEASE_LABEL\s*=\s*'v91\.13 · 2026-09-08'/);
...
(async()=>{
  ...
  console.log('PASS SW v91.13 starts cached navigation/code without waiting for network, refreshes in background and preserves application data');
