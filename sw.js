const CACHE_NAME = 'maister-treker-v50'; // NEW: 1) текст наряду при створенні заявки тепер потрапляє в приватну примітку ("🔒 Тільки для вас"), а не в поле, яке летить диспетчеру — раніше диспетчеру могло повернутись його ж повідомлення; 2) кнопки "✈️ Диспетчеру" і "📄 Копіювати" прибрано з картки заявки в загальному списку (менше кнопок, менша картка) — у профілі абонента вони лишились.
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
