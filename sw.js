const CACHE_NAME = 'maister-treker-v46'; // NEW: масова (повна) синхронізація з Google Таблицею (кнопка "Надіслати всі") тепер пише всі заявки/зміни ОДНИМ пакетним викликом замість окремого звернення до Sheets API на кожен рядок — при 500-1000+ заявках попередній варіант міг упертись у ліміт часу виконання Apps Script (6 хв) і лишити лист частково записаним. Стосується коду, який копіюється в Google Apps Script (кнопка "Копіювати код" у Налаштуваннях) — після оновлення файлу треба ще раз скопіювати код у сам Apps Script і зберегти там.
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
