const CACHE_NAME = 'maister-treker-v41'; // NEW: 1) фото — повернуто окрему кнопку "Камера" (capture=environment) поряд з "Галерея" (multiple без capture): на Android Chrome ці два атрибути разом на одному input прибирали пункт "Камера" з системного вибору, тепер знімок можна зробити прямо з застосунку, а не лише вибрати з галереї. 2) Оплата "Безкоштовно" тепер справді обнуляє суму заявки (раніше сума й далі рахувалась як завжди, "Безкоштовно" на неї не впливало). 3) Профіль абонента (навігатор адрес) тепер показує фото з УСІХ заявок за адресою галереєю (раніше — лише одне, найсвіжіше), і в Telegram-бекап тепер летять усі фото заявки (раніше — лише перше).
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
