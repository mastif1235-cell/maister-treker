const CACHE_NAME = 'maister-treker-v32'; // NEW: прибрано обладнання з шапки профілю (лишили лише в самій картці заявки, як і просили). Головне — знайдено й виправлено реальний баг з часів рефакторингу зберігання: картки заявок у профілі й місячний звіт по матеріалах перестали показувати обладнання/роботи взагалі, бо шукали поле checked, яке ми свідомо перестали зберігати (сама присутність позиції в масиві й означає "вибрано"). Виправлено в обох місцях, сумісно і зі старими, і з новими заявками — перевірено тестом.
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
