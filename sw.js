const CACHE_NAME = 'maister-treker-v55'; // NEW: 1) видалення заявки без інтернету більше не "губиться" назавжди — якщо запит на видалення в Google Таблицю не пройшов одразу, тепер він повторюється автоматично (кнопка "Повторити" / поява інтернету), раніше рядок у Таблиці міг лишитись назавжди, бо видалена заявка зникала з черги повторної синхронізації; 2) якщо запис фото в IndexedDB не вдався — тепер одразу з'являється попередження, а не мовчазна втрата фото при наступному відкритті застосунку.
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
