const CACHE_NAME = 'maister-treker-v35'; // NEW: захист входу в застосунок — пароль (зберігається як SHA-256 хеш, не відкритим текстом) + опційно відбиток пальця через WebAuthn поверх нього (якщо не спрацює/скасовано — одразу видно поле пароля). Налаштовується в Налаштування → 🔒 Захист входу. Це бар'єр від чужого погляду на загублений/вкрадений телефон, а не криптографічний захист від технічного втручання.
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
