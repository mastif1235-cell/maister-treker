const CACHE_NAME = 'maister-treker-v45'; // NEW: виправлення за результатами зовнішнього аудиту — 1) текст наряду від диспетчера більше не губиться при збереженні заявки (тепер у видимому полі "Примітка"); 2) "Безкоштовно" тепер зануляє ціни і в профілі абонента, не лише в тексті заявки; 3) фото одразу переносяться в IndexedDB замість localStorage — чернетка більше не може "тихо" не зберегтись через переповнення сховища; 4) чернетка коректно зберігає правки заявок, відновлених із хмари; 5) додано таймаут (20с/15с) на мережеві запити — збереження заявки більше не може зависнути на невизначений час при поганому зв'язку.
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
