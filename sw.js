const CACHE_NAME = 'maister-treker-v60'; // NEW: 1) у тексті заявки/профілі абонента розбивка "Змішана оплата" тепер показує ЩО саме куплено готівкою, а що безготівкою (не лише дві суми); 2) розпізнавання номера телефону з тексту наряду тепер шукає саме формат українського номера, а не будь-яку послідовність цифр — раніше міг випадково підхопити номер договору чи інше число; 3) збереження заявки більше не чекає відповіді Google Таблиці перед поверненням до списку — синхронізація йде повністю у фоні (як і бекап у Telegram), тому "холодний старт" Apps Script (кілька секунд на перший запит після паузи) більше не виглядає як зависання застосунку.
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
