const CACHE_NAME = 'maister-treker-v33'; // NEW: діаграма "Огляд за місяцями" тепер гортається вбік замість вилазити за рамку картки; у списку змін за місяць видно заробіток за конкретний день (лише в застосунку, у звіт/Telegram не потрапляє); виправлено "Сам" — раніше воно було приховано "вшите" у вибір напарників і лишалось поруч з реальним напарником навіть після вибору когось іншого (тепер вибір стартує порожнім, "Сам" підставляється лише як текст, якщо нікого не обрано); розділ "Теги" в калькуляторі згорнуто в акордеон, як і решта розділів (Обладнання/Кабелі/Роботи) — раніше він завжди був розгорнутий.
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
