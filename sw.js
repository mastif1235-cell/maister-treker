const CACHE_NAME = 'maister-treker-v37'; // NEW: при додаванні наряду в чергу тепер одразу перевіряється, чи це знайомий абонент (за телефоном або адресою з тексту) — переиспользано ту саму логіку й вигляд карток, що й у "Перевірити наряд". Наряд у будь-якому разі додається; якщо знайшлись збіги — показуються окремим екраном із кнопкою переходу в профіль абонента (переглянути попередні заявки), звідки повертає назад у чергу нарядів.
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
