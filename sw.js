const CACHE_NAME = 'maister-treker-v43'; // NEW: при оплаті "Безкоштовно" з тексту заявки (для диспетчера/Telegram) прибрано порядкові суми (виклик/тариф/обладнання/кабелі/роботи) — раніше вони й далі показувались як звичайні числа поряд з "ІТОГО: 0 грн", що виглядало суперечливо. Позиції (що зроблено) лишаються, тільки без цін.
const CORE_ASSETS = [
  './',
  './index.html',
  './styles.css',
