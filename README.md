...
- Адресний довідник Stage 2A–2D: локальні UUID міст/вулиць, явні aliases та архів; заявки отримують необов'язкові `cityId`/`streetId` (нова чи відредагована адреса або явна «Перевірити адреси заявок» у налаштуваннях), зв'язок іде за адресою заявки, legacy-заявки без UUID працюють без змін; довідник автоматично передається AI-бекенду (`POST /directory` → KV), тож AI розрізняє ДОВІДНИК («які вулиці є») і ЗАЯВКИ («де були заявки») та шукає за UUID. Модель, безпечна міграція та межі: [`docs/STAGE2_ADDRESSBOOK.md`](docs/STAGE2_ADDRESSBOOK.md).
...
«Свій backend» — будь-який ваш https-Worker з тим самим контрактом (`/healthz`, `/ai/config`, `/ask`, `/directory` — приймає довідник адрес для Stage 2D). http заборонено; відповідальність за безпеку — на власника.
